...
costo_totale_mozzarella = costo_mozzarella * (quantita_venduta * 0.5) * numero_consegne  # Costo totale per kg
prezzo_totale_vendite = prezzo_vendita * (quantita_venduta * 0.5) * numero_consegne  # Prezzo totale per kg
...
costo_invenduto = costo_mozzarella * (quantita_venduta * 0.5) * (percentuale_invenduto / 100) * numero_consegne  # Costo invenduto
...
if st.button("Calcola BEP"):
    if guadagno_netto != 0:
        ...
        bep_percentuale_invenduto = (guadagno_netto / (costo_mozzarella * (quantita_venduta * 0.5) * numero_consegne)) * 100  # Percentuale invenduto per pareggio
...
for q in quantita_range:
    costo_totale_mozzarella = costo_mozzarella * (q * 0.5) * numero_consegne  # Costo totale per kg
    prezzo_totale_vendite = prezzo_vendita * (q * 0.5) * numero_consegne  # Prezzo totale per kg
    guadagno_netto = prezzo_totale_vendite - costo_totale_mozzarella - costo_totale_consegna
    costo_invenduto = costo_mozzarella * (q * 0.5) * (percentuale_invenduto / 100) * numero_consegne  # Costo invenduto
